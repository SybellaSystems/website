    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "nav-blur" : "bg-transparent"}`}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(12px, 4vw, 32px)", height: "clamp(56px, 10vw, 72px)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: "clamp(12px, 2vw, 16px)", textDecoration: "none" }}>
          <div style={{ width: "clamp(40px, 8vw, 48px)", height: "clamp(40px, 8vw, 48px)", borderRadius: 12, background: "linear-gradient(135deg, rgba(201,168,76,0.1), rgba(59,130,246,0.1))", border: "1px solid rgba(201,168,76,0.2)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", boxShadow: "0 4px 20px rgba(201,168,76,0.15)", flexShrink: 0 }}>
            <img src="/LOGO WITH NO BG.png" alt="Sybella Systems Logo" style={{ width: "clamp(28px, 6vw, 32px)", height: "clamp(28px, 6vw, 32px)", objectFit: "contain", filter: "brightness(0) invert(1)" }} />
          </div>
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 4 }} className="hidden lg:flex">
          {links.map(l => (
            <Link
              key={l.href}
              href={l.href}
              style={{ padding: "8px 16px", fontFamily: "var(--font-display)", fontSize: "clamp(11px, 1.5vw, 13px)", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: pathname === l.href ? "var(--blue-bright)" : "var(--text-secondary)", transition: "color 0.2s", borderRadius: 2, textDecoration: "none", minHeight: "44px", display: "flex", alignItems: "center" }}
