...
async function seedAdmin() {
  ...
  try {
    ...
    const existingAdmin = await collection.findOne({ email: DEFAULT_ADMIN.email });
    if (existingAdmin) {
      console.log('⚠️  Admin user already exists!');
      console.log(`   Email: ${DEFAULT_ADMIN.email}`);
      console.log('\n📝 If you forgot your password, you can:');
      console.log('   1. Reset it through the database');
      console.log('   2. Or delete the existing admin and run this script again');
      await client.close();
      return;
    }
    ...
    if (staffCount > 0) {
      console.log(`⚠️  Warning: ${staffCount} staff member(s) already exist in the database.`);
